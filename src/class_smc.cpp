...
void class_smc::init(){ //Initialization
  ...
  if(method == 1){
    ...
    pre_compute_OTHER(G, I, NL, model_parameters_list);
  }
...
void class_smc::make_xi_cube(){ //Pre-compute xi
  ...
  for(int k=0;k<max_K;k++){
    ...
    if(method == 1){
      ...
      xi_cube.slice(k) = CreateXi_OTHER(I, model_parameters_list, k);
    }
  }
}
    ...
    for(int v=1;v<I+1;v++){
      ...
      for(int l=0;l<n_location_options;l++){
        ...
        if(method == 1){
          ...
          log_ML(l) = log_ML_compute_OTHER(G, I, count_cube_l.tube(j,l), count_cube_r.tube(j,l), L_points(l), NL, v, k_current);
        }
      }
    ...
    if(children_IDs_i(0,j) == -1){
      ...
      for(int v=1;v<(I+1);v++)
      {
        ...
        if(method == 1){
          ...
          log_phi_post(v-1,j) = log_ML_compute_OTHER(G, I, n_obs_i.col(child_ID_l), n_obs_i.col(child_ID_r), L_child, NL, v, k_j) +
            log_Phi_post(v-1,child_ID_l) + log_Phi_post(v-1,child_ID_r);
        }
      ...
      if(children_IDs_i(0,j) != -1){
        ...
        for(int v=1;v<I+1;v++){
          if(method == 1){
            ...
            mean_temp(v-1) = PostMean_OTHER(I, n_obs_i.col(child_ID_l), n_obs_i.col(child_ID_r), L_temp, v, g, k_j);
          }
        }
